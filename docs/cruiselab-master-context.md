...
Last updated: 15 July 2026 (rev 16). This document is the single source of truth for The Cruise Lab initiative — every property, where it lives, how it is built and deployed. All enhancements, new tools, and maintenance work should be consistent with it; update it whenever the estate changes. Changes in rev 16: **Shore Thing grew and gained editorial.** Its port database went from 260 to **302 ports** (batch 2 added the southern-South-America circuit, all four Hawaiian islands, New Zealand, the South Pacific, more of Australia, and southern/Indian-Ocean Africa; a misfiled "Sydney, Nova Scotia" was moved from Oceania to North America). Editorial coverage doubled to **179 entries — the Americas AND all of Europe complete** (103 European ports written this session; nine stragglers and three invented-port orphans caught and fixed in validation). Remaining editorial: Asia 67 · Oceania 27 · S. America 16 · Africa 11. The GetMyCruiseWeather backport task grew to **86 `gw:false` ports** and now also carries the Sydney-NS region-bug fix (the same misfile exists in GetMyCruiseWeather's own ports.json). Everything else below — the social estate, link hub, email routing, campaign — is **carried forward unchanged from rev 15**, which recorded: **the social estate is live and audited.** TikTok, Instagram, YouTube and Facebook accounts all exist under `@thecruiselabhq`; Facebook's username was set and its bio repaired on 14 Jul; a per-platform audit found only two open TikTok fixes. The **link hub is live** at links.thecruiselab.com and **Cloudflare Email Routing is done** (`hello@` and `social@` both live; Forever Voyage's `CONTACT_EMAIL` switched to `hello@` in the repo). Two new deliverables: the **launch marketing campaign** doc (`cruiselab-launch-campaign-v1-20260714.md` — the structured post sequence; companion to the playbook) and the **intro promo video** (`cruiselab-intro-promo-v1-20260714.mp4`, 40s 9:16, with its Python renderer). New lesson recorded: Claude's web fetch can serve *days*-stale copies of Cruise Lab pages even with cache-busting — only Stuart's curl/browser verifies a deploy. NOTE: much of this work was first done in a chat that failed and was deleted; this rev is the reconstruction of record. Three link-hub plumbing facts are marked **TO CONFIRM** below. *Still deferred:* the Ocean View v2 write-up (YouTube harvest in progress) — the Ocean View entry is carried forward unchanged from rev 13.
...
- **Editorial coverage: the Americas AND all of Europe complete (179 entries as of 15 Jul).** Europe was written as three sub-region batches (British Isles/Nordic/Baltic, Iberia/France, Italy/Adriatic/Greece/Turkey/islands) covering all 103 European port ids incl. the river-cruise calls (Lyon, Vienna, Verona framed honestly) and the duplicate id pairs (Edinburgh Leith + South Queensferry, Livorno/Florence ×2, Porto ×2, Cadiz/Seville). Remaining batches queued: **Asia (67), Oceania (27), S. America (16), Africa (11)** — 121 ports. Lesson from the Europe batch: validate every editorial key against `ports.json` before merging — three entries were written for ports that don't exist in the DB (Dartmouth, Santander) or duplicated an existing id (Koper → `ljubljana-koper`) and had to be removed.
...
- Verified at launch by jsdom smoke test (23/23).
...
- **Shore Thing editorial batches**: Americas and Europe now complete (179 entries). Remaining: **Asia (67) next, then Oceania (27) / S. America (16) / Africa (11)** — 121 ports. Drop entries into `data/guide-content.json` — no code changes needed; always validate keys against `ports.json` before merging (see the tool entry's lesson).
