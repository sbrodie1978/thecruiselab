...
Last updated: 15 July 2026 (rev 17). This document is the single source of truth for The Cruise Lab initiative — every property, where it lives, how it is built and deployed. All enhancements, new tools, and maintenance work should be consistent with it; update it whenever the estate changes. Changes in rev 17: **Shore Thing editorial is now COMPLETE — all 302 ports, every region.** The final 123 entries were written in one session (the Hawaii/Philadelphia/Sydney-NS stragglers, the full South American southern circuit, all of Asia's 67, all of Oceania's 27, all of Africa's 11), keyed and validated against `ports.json` with one orphan caught in a pre-merge dry run (`koh-chang` → the real id is `trat-koh-chang`) and removed. Inland/transfer ports (Beijing/Tianjin, Bangkok/Laem Chabang, HCMC/Phu My, Chiang Mai as a fly-in) are framed honestly rather than as walk-off calls; practical warnings (China visas, morning-only mosque entry in the Gulf, modest-dress and tender/weather caveats) are baked into the relevant notes. Verified by a 7-port world-cruise jsdom smoke test (10/10). The editorial backlog item is now closed. Everything else — the 302-port database, the GetMyCruiseWeather backport task (still 86 `gw:false` ports + the Sydney-NS fix), the social estate, link hub, email routing, campaign — is **carried forward unchanged from rev 16**, which recorded: Shore Thing's port database reaching **302 ports** (batch 2 adding S. America/Hawaii/NZ/Pacific/Africa) and editorial through the Americas and Europe; and, from rev 15: **the social estate is live and audited.** TikTok, Instagram, YouTube and Facebook accounts all exist under `@thecruiselabhq`; Facebook's username was set and its bio repaired on 14 Jul; a per-platform audit found only two open TikTok fixes. The **link hub is live** at links.thecruiselab.com and **Cloudflare Email Routing is done** (`hello@` and `social@` both live; Forever Voyage's `CONTACT_EMAIL` switched to `hello@` in the repo). Two rev-15 deliverables: the **launch marketing campaign** doc (`cruiselab-launch-campaign-v1-20260714.md`) and the **intro promo video** (`cruiselab-intro-promo-v1-20260714.mp4`, 40s 9:16, with its Python renderer). Standing lesson: Claude's web fetch can serve *days*-stale copies of Cruise Lab pages even with cache-busting — only Stuart's curl/browser verifies a deploy (a `?v=$(date +%s)` cache-buster on the curl also defeats it). Three link-hub plumbing facts remain **TO CONFIRM** below. *Still deferred:* the Ocean View v2 write-up (YouTube harvest in progress) — the Ocean View entry is carried forward unchanged from rev 13.
...
- **Shore Thing editorial: COMPLETE** (all 302 ports have entries as of 15 Jul). Future work here is only refinement/updates, not net-new coverage — until new ports are added, in which case each needs both a `ports.json` record and a validated `guide-content.json` entry.
